  ...
  return (
    <DropdownMenu open={isOpen} onOpenChange={() => setIsOpen((prev) => !prev)} modal={isOpen}>
      ...
      <DropdownMenuContent align="end">
        ...
        <AlertDialog>
          <AlertDialogTrigger className={`w-full bg-destructive text-white p-2 text-sm rounded-md ${appointment?.status == 'cancelled' ? 'bg-muted booked-cursor dark:!text-white !text-black' : 'hover:bg-destructive/90'}  ${appointment?.status == 'cancelled' || appointment?.status == 'completed' ? 'cursor-not-allowed' : ''}`} disabled={appointment?.status == 'cancelled' || appointment?.status == 'completed'}>Cancel</AlertDialogTrigger>
          <AlertDialogContent>
            <AlertDialogHeader>
